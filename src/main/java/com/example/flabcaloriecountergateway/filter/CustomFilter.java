...
public class CustomFilter extends AbstractGatewayFilterFactory<CustomFilter.CustomFilterConfig> {
    ...
    public GatewayFilter apply(CustomFilterConfig config) {
        return ((exchange, chain) -> {
            ...
            logger.atInfo().log("Custom PRE filter: request id -> {}", request.getId());

            // Post Filter
            return chain.filter(exchange).then(
                    Mono.fromRunnable(() -> logger.atInfo().log("Custom POST filter: response code -> {}", response.getStatusCode()))
            );
        });
    }
